...
struct sock_data{
	...
	: m_adr(adr), m_adr_l(adr_l), m_sock_udp(sock_udp), m_sock_tcp(sock_tcp), m_peer_info(peer_info), m_contype(type)
	{}

	sock_data(int * sock_tcp, int * sock_udp, peer_pair_t peer_info, con_type type)
:m_sock_udp(sock_udp), m_sock_tcp(sock_tcp), m_peer_info(peer_info), m_contype(type)
	{
	...
	int write(std::string data, bool isTCP=false){
		...
		if( isTCP && (m_contype&con_type::tcp != con_type::tcp) ) throw("TCP with peer unnallowed");
	 	if( !isTCP && (m_contype&con_type::udp != con_type::udp) ) throw("UDP with peer unnallowed");
		int c = sendto(tmp, data.c_str(), data.size(), 0, (struct sockaddr *)&m_adr,
         		m_adr_l);
		if( c < 0 ) throw( std::runtime_error("Can't send to peer"+ (m_peer_info.first+":"+std::to_string(m_peer_info.second)) ));
		return c;
	}
	...
	msg_pair_t read(size_t sbuf=1024, bool isTCP=false){
		...
		int c = recvfrom(tmp, buf, sbuf, O_NONBLOCK, (struct sockaddr *)&tadr,
            		 &tadr_l);
		if( c < 0 ) throw( std::runtime_error("Can't recv from peer "+ (m_peer_info.first+":"+std::to_string(m_peer_info.second)) ));
		sock_data n;
		if( !operator==(tadr) ){
		        const char *ip = inet_ntoa(tadr.sin_addr);
		        const uint16_t port = htons(tadr.sin_port);
			 n = sock_data(
				 m_sock_tcp, m_sock_udp, {ip, port}, con_type::udp);
		}
		else
		n = sock_data(
				 m_sock_tcp, m_sock_udp, m_peer_info, con_type::udp);
...
struct serv_data{
	...
	void self_bind(void){
		  ...
		  if(m_udp_self <=0) throw( std::runtime_error("Can't init sockets (SELF)") );
...
try{
		  ...
		  puts("users list:");
		  auto addresses = split::split((char*)user_list_data.c_str(), ';', user_list_data.size() );
		  for(auto address : addresses){
			...
			sock_data n_data = sock_data(
				 &m_sock_tcp, &m_sock_udp, {ip, atoi(port.c_str())}, con_type::udp);
	...
	serv_data(std::string host, uint16_t port): m_contype(unconnected)
	{
		...
		if( m_sock_udp <= 0 || m_sock_tcp <= 0 ){
			if(m_sock_udp > 0) close(m_sock_udp);
			if(m_sock_tcp > 0) close(m_sock_tcp);
		...
		if(ret >= 0) m_contype = (con_type)(m_contype|con_type::udp);
		else{
		 std::cout << "Can't connect to UDP"<<std::endl;
		 close(m_sock_udp);
		}
		ret = connect(m_sock_tcp, (struct sockaddr *)&addr, addr_l);
		if(ret >= 0) m_contype = (con_type)(m_contype|con_type::tcp);
		else{
		 ...
		 close(m_sock_tcp);
  		...
  		if (m_contype == con_type::unconnected) {
			//closeconn();
			char errbuf[256];sprintf(errbuf,"Can't connect to %s:%d\n%c", host.c_str(), port, 0);
   		 	throw( std::runtime_error(  errbuf  ) );
  		}//connected
		m_data = sock_data(addr, addr_l, &m_sock_tcp, &m_sock_udp, {host, port}, m_contype); 
	}
	int write(std::string data, bool isTCP=false){
	...
	msg_pair_t read(peer_pair_t peer_info, size_t sbuf=1024, bool isTCP=false){

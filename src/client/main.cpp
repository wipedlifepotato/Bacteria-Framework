...
int main(int argc, char ** argv) {
	if(argc != 3) return fprintf(stderr,"%s host port\n", argv[0]);
	int udp_sock_serv = socket(AF_INET, SOCK_DGRAM, 0);
	int udp_sock_self = socket(AF_INET, SOCK_DGRAM, 0);
	if(udp_sock_serv < 0 || udp_sock_self < 0) return fprintf(stderr, "Can't init sockets\n");
	...
	sendto(udp_sock_serv, buf, strlen(buf), 0, (struct sockaddr*)&serv_addr, socklen );
	bzero(buf,sizeof(buf));
	recvfrom(udp_sock_serv, buf, sizeof(buf), 0,(struct sockaddr*)&serv_addr, &socklen);
	if(strlen(buf) == 0){
	...
	std::string sPort = t.substr(t.find(':')+1, t.find(']')-(t.find(':')+1) );
	std::cout <<"Self IP: " << sIp << std::endl << "Self port: " << sPort << std::endl; 
	bzero(buf,sizeof(buf));
	recvfrom(udp_sock_serv, buf, sizeof(buf), 0,(struct sockaddr*)&serv_addr, &socklen);
...
\
		...
		sendto(udp_sock_self, p.c_str(), p.size(), 0,\
			reinterpret_cast<struct sockaddr*>(&him_addr), sizeof(struct sockaddr_in) );\
	}
	...
	for(;;){
		...
		socklen_t hadr_l;
		recvfrom(udp_sock_self, buf, sizeof(buf),0, (struct sockaddr*)&hadr, &hadr_l);
		if(strlen(buf) > 0){
			puts("Buffer:");
			...
			printf("%s:%d %s\n", ip, port, buf);
			sendto(udp_sock_self, p.c_str(), p.size(), 0,\
			reinterpret_cast<struct sockaddr*>(&hadr), sizeof(struct sockaddr_in) );
		}
		//puts("usleep");
		usleep(500);
		//puts("endfor");
...
/*
 ...
 int ret = bind(main_descriptor_udp, (struct sockaddr *)&my_addr,
             sizeof(struct sockaddr_in));
  if (ret == -1) {
    return fprintf("Can't init bind on (UDP) %s:%d\n", host, port);
  }

  ...
  public:
    void generateKeys(std::string filepath = "", unsigned int bits = 8192,
      ...
      if (filepath != "") {
        ...
        if (exitFile == nullptr)
          throw std::runtime_error("Can't save rsa/ed25519 key to: " +
                                   filepath);
      }
      if (mType == ed25519) {
    ...
    virtual bool verify(ustring data, ustring pubkey, size_t keySize = len_key,
      ...
      auto rt = ed25519rsa_verifyIt(
          ...
          data.size(), const_cast<uint8_t *>(pubkey.c_str()), pubkey.size(),
          mdalgo, mType);
      if (rt > 0)
        return true;
      return false;
    ...
    sign(string data, size_t retSize = 65,
         const EVP_MD *mdalgo = EVP_sha512()) { // sha512 512/8 = 64

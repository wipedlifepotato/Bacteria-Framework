...
namespace FileSystem{
	...
	namespace size{
		sfPair_t
		isOf(double long bytes){ //change name
    ...
    class dir{
	    ...
	    ~dir(void){
            ...
            {
                if(m_dir == nullptr) throw( ("Can't open dir "+dir) );
            ...
            filepair_t get(std::string path,DIR * t){
                ...
                while( (d=readdir( t )) ){
                        ...
                        if(d->d_type == DT_DIR){
                            if( std::string(d->d_name) == "." || (strcmp(d->d_name, "..") == 0) ) continue;
            ...
            filepair_t get(void){
                    return get(m_namedir, m_dir);
            }
            filepair_t get(std::string path){
                if( path == "." || path == ".." ) return {};
                DIR * tmp = opendir( path.c_str() );
                if( tmp == nullptr ) throw( "Can't open dir "+path );
                return get(path,tmp);
                closedir(tmp);
            }
...
void usage(void){
	puts(
		__DATE__ " deletor dada; usage: ./dada [options] path to dir\n"
...
static struct option long_options[] = {
  ...
  while( (             c = getopt_long(argc, const_cast<char**>(argv), "s:hz",
...
//geopt...
if(maxSize == 0) return fprintf(stderr, "[%s -h]\n", argv[0]);
...
try{
	...
	std::cout << "Size in bytes: " << file.second << std::endl;
        std::cout << "Size of file: " << sOf.second << " " << sOf.first << std::endl;
	...
	for(unsigned int i = 60;i--;) printf("%c", '~');
	puts("");
    }
}catch(std::string e){
        std::cerr << e << std::endl;

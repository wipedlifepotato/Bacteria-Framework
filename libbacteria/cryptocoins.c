...
struct cryptocoin *init_new_cryptocoins(const char *path,
                                        uint *size) { // not thread safe.
...
#define SET_VALUE_string(what)                                                 \
  ...
  struct cryptocoin *cryptocoins =
      calloc(sizeof(struct cryptocoin), count_cryptocoins);
  if (cryptocoins == NULL) {
...
void dump_cryptocoins(struct cryptocoin *cryptocoins) {
  for (unsigned char i = count_cryptocoins;
       i--;) { // cryptocoins[i].rpchost != NULL && cryptocoins[i].rpcport !=
               // 0;i++){
    printf("Cryptocoin - %s\n", cryptocoins[i].cryptocoin_name);
    printf("%s:%d %s:%s testnet:%s\n", cryptocoins[i].rpchost,
           cryptocoins[i].rpcport, cryptocoins[i].rpcuser,
           cryptocoins[i].rpcpassword,
           cryptocoins[i].testnet ? "is testnet" : "is not testnet");
  }
}

void clear_cryptocoins(struct cryptocoin *cryptocoins) {
  for (unsigned char i = 0;
       cryptocoins[i].rpchost != NULL && cryptocoins[i].rpcport != 0 &&
       i < count_cryptocoins;
       i++) {

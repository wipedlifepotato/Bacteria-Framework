...
int AES_any_decrypt(aes_enctype enc, unsigned char *ciphertext,
...
int AES_any_encrypt(aes_enctype enc, unsigned char *plaintext,
...
static int libbacteria_AES_test(int count, char **values) {
  ...
  unsigned char *plaintext = (unsigned char *)values[1];

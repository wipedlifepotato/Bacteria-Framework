...
INITLUAFUNC(send) {
  ...
  struct sockaddr *dadr;
  socklen_t addrlen = sizeof(struct sockaddr_in);
  if (sock <= 0)
    luaL_error(L, "Socket is bad");
  if (cTop >= 3)
    dadr = (struct sockaddr *)lua_touserdata(L, 3);
  if (cTop >= 4)
    addrlen = lua_tonumber(L, 4);
  if (cTop == 5)
    flags = flags;
  size_t smsg = strlen(msg);
  int r = dadr == NULL ? send(sock, msg, smsg, flags)
                       : sendto(sock, msg, smsg, flags, dadr, addrlen);
  if (r <= 0)
    lua_pushnil(L);
  else
    lua_pushboolean(L, 1);
...
INITLUAFUNC(recv) {
  ...
  size_t r = dadr == NULL
                 ? recv(sock, buf, recv_count, flags)
                 : recvfrom(sock, buf, recv_count, flags, dadr, &addrlen);
  if (r <= 0) {
    lua_pushnil(L);
    return 1;
  }
...
INITLUAFUNC(getsockaddr) {
  ...
  if (lua_gettop(L) == 3)
    sin_family = lua_tonumber(L, 3);

  size_t nbytes = sizeof(struct sockaddr_in) + sizeof(int) * (sin_family - 1) +
                  sizeof(in_port_t) * (port - 1) +
                  sizeof(uint32_t) * (s_addr - 1);
...
INITLUAFUNC(accept) {
  int sock = lua_tonumber(L, 1);
  if (sock <= 0)
    luaL_error(L, "Bad sock descriptor for accept");
  ...
  size_t nbytes = sizeof(struct sockaddr_in) +
                  sizeof(int) * (addr.sin_family - 1) +
                  sizeof(in_port_t) * (addr.sin_port - 1) +
                  sizeof(uint32_t) * (addr.sin_addr.s_addr - 1);
  struct sockaddr_in *rr = (struct sockaddr_in *)lua_newuserdata(L, nbytes);
...
INITLUAFUNC(bind) {
  ...
  if (adr == NULL)
    luaL_error(L, "bad bind addr (use lnet.getsockaddr(host,port) as example "
                  "before [sin_family optional as third argument])");
  if (cArgs >= 2)
    useTCP = lua_toboolean(L, 2);
  if (cArgs >= 3)
    useUDP = lua_toboolean(L, 3);
  if (cArgs >= 4)
    listen_count = lua_tonumber(L, 4);
  if (cArgs >= 5)
    socklen = lua_tonumber(L, 5);
  if (useTCP) {
  ...
  if (useUDP)
    lua_pushnumber(L, sock_udp);
  if (useTCP)
    lua_pushnumber(L, sock_tcp);
  if (useUDP && useTCP)
    return 2;
  return 1;
}
INITLUAFUNC(close_sock) {
  int sock = lua_tonumber(L, 1);
  if (sock <= 0)
    luaL_error(L, "Bad socket for close");
  close(sock);
  lua_pushboolean(L, 1);
  return 1;
}
...
INITLUAFUNC(unpackData) {
  // char ** unpackData(char * data, size_t * rt_size, char fsplitter, char
  // ssplitter){
  ...
  size_t cArgs = lua_gettop(L);
  if (cArgs >= 2)
    fsplitter = lua_tostring(L, 2)[0];
  if (cArgs >= 3)
    ssplitter = lua_tostring(L, 3)[0];

  size_t rt_size;
  char **rv = unpackData((char *)packedData, &rt_size, fsplitter, ssplitter);
  if (rv == NULL)
    luaL_error(L, "bad packed data");
...
INITLUAFUNC(packData) {
  // char * packData(char ** dataKeys, char ** dataValues, char fsplitter, char
  // ssplitter){ const char *
